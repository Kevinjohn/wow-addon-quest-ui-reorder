...
ns.L = setmetatable({
    ...
    OPTION_SPLIT_WARNING_121 = "Warning: because of a bug in patch 12.1, the quest tracker does not update while this is on — you have to use /reload to see changes. Blizzard is expected to fix this in 12.1.5. Best left off until then.",
    MSG_SPLIT_RESET_121 = "quest sections have been turned off: a bug in patch 12.1 stopped the quest tracker updating while they were on. Sorting still works. You can turn them back on in the addon's options.",
    MSG_DISABLED_121_TAINT = "disabled on patch 12.1: a Blizzard bug freezes the whole quest tracker when an addon changes it, so nothing is hooked. The addon will start working again on a patch that fixes it.",
    OPTION_SPLIT_OVERRIDE_121 = "Ticking this runs the addon anyway, after a /reload. The quest tracker will then stop updating until you reload it again.",
