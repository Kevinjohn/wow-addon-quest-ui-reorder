...
ns.L = setmetatable({
    ...
    OPTION_SORT_LABEL = "Order quests by type",
    OPTION_SORT_TOOLTIP = "Order tracked quests by type: Important, Legendary, Meta, Repeatable, Storyline, then everything else.",
    OPTION_121_NOTE = "Note for patch 12.1: Blizzard's tracker still checks your buffs every time it redraws, left over from Shadowlands. In 12.1 they locked buff data down, so once an addon touches the tracker that check can fail and the tracker stops updating until you /reload.",
    OPTION_NEEDS_RELOAD = "Changing this takes effect after a /reload.",
    MSG_OPT_IN_RESET = "both features are now off until you turn them on: see this addon's options for what each one does on patch 12.1.",
